...
def register():
	if request.method=="POST":
		...
		session.add(owner)
		session.commit()
		flash("owner is sucessfully created",'info') 
...
def logout():
	...
	flash('logout success and visit again','success')
	return redirect(url_for('home'))
...
def newpost():
	if request.method=="POST":
		...
		session.add(post)
		session.commit()
		flash("sucessfully Posted",'success')
...
def editpost(post_id):
	if request.method == 'POST':
		...
		session.add(post)
		session.commit()
		flash("sucessfully updated post",'success')
		return redirect(url_for('home'))
	else:
		post=session.query(Post).filter_by(id=post_id).one_or_none()
